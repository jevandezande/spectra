...
def test_init():
    ...
    s1 = ContinuousSpectrum("Hello World", energies, intensities, units="ms", style="IR")
...
def test_iter():
    energies, intensities = np.arange(10), np.arange(10)
    s1 = ContinuousSpectrum("Hello World", energies, intensities)
...
def test_eq():
    energies, intensities = np.arange(10), np.arange(10)
    s1 = ContinuousSpectrum("S1", energies, intensities)
    s2 = ContinuousSpectrum("S1", energies, intensities)
    s3 = ContinuousSpectrum("S3", energies, intensities)
    s4 = ContinuousSpectrum("S4", energies, energies)
    s5 = ContinuousSpectrum("S5", intensities, intensities)
...
def test_len():
    energies, intensities = np.arange(10), np.arange(10)
    s1 = ContinuousSpectrum("Hello World", energies, intensities)
    s2 = ContinuousSpectrum("Hello World", intensities, energies)
...
def test_str():
    ...
    assert str(s1) == "<ContinuousSpectrum: Hello World>"


def test_add_sub():
    energies1, intensities1 = np.arange(10), np.arange(10)
    energies2, intensities2 = np.arange(20), np.arange(20)
    s1 = ContinuousSpectrum("Hello World", energies1, intensities1)
    s1 + s1
    s2 = 1 + s1
    s3 = s2 - 1
    s4 = 1 - s3
...
def test_abs():
    ...
    s1 = ContinuousSpectrum("Hello World", energies, intensities)
    s2 = ContinuousSpectrum("Hello World", energies, intensities2)
...
def test_div():
    ...
    s3 = s1 / 2
    s4 = s1 / s1
    s5 = ContinuousSpectrum("Hello World", energies1, np.array([1] * 10))
    s6 = s1 / s2
    s7 = ContinuousSpectrum("Hello Big World", energies2, intensities2)
...
def test_sliced():
    ...
    s1 = ContinuousSpectrum("Hello World", energies, intensities)
    s2 = ContinuousSpectrum("Hello World", energies[5:], intensities2)
    s3 = ContinuousSpectrum("Hello World", energies[:5], intensities3)
...
def test_from_csvs(tmp_path):
    ...
    with open(test_csv, "w") as f:
        f.write("x,A,B\n0,2,4\n1,3,5")
    ContinuousSpectrum.from_csvs(test_csv)
    ContinuousSpectrum.from_csvs("tests/files/xrd.csv")


def test_norm():
    energies, intensities = np.arange(10), np.arange(1, 11)
    s1 = ContinuousSpectrum("1", energies, intensities)
...
def test_correlation():
    energies, intensities = np.arange(10), np.arange(1, 11)
    s1 = ContinuousSpectrum("1", energies, intensities)
    s2 = ContinuousSpectrum("2", np.array([0, 1, 2, 3]), np.array([1, 0, 1, 0]))
    s3 = ContinuousSpectrum("3", np.array([0, 1, 2, 3]), np.array([1, 1, 0, 0]))
